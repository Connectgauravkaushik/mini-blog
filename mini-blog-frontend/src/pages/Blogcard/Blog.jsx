...
const BlogPost = ({ blogs: blogsProp } = {}) => {
  ...
  const matchesId = (b, rid) => {
    ...
    const titleEncoded = b.title
      ? encodeURIComponent(String(b.title).toLowerCase())
      : "";
  ...
  const ui = useMemo(() => {
    ...
    const authorName =
      (blog.author && (blog.author.fullName || blog.author.name)) ||
      blog.fullName ||
      "Unknown Author";
  ...
  const related = useMemo(() => {
    const source =
      Array.isArray(blogsProp) && blogsProp.length > 0
        ? blogsProp
    ...
    for (let i = 0; i < pool.length && result.length < 3; i += 1) {
      ...
      const candId = String(
        candidate._id || candidate.id || candidate.slug || candidate.title || ""
      );
      const currentId = String(
        blog?._id || blog?.id || blog?.slug || blog?.title || ""
      );
    ...
    if (result.length < 3) {
      ...
      for (let i = 0; i < extraSources.length && result.length < 3; i += 1) {
        ...
        const already = result.some(
          (r) => String(r._id || r.id || r.slug || r.title || "") === candId
        );
        const currentId = String(
          blog?._id || blog?.id || blog?.slug || blog?.title || ""
        );
        ...
        {!blog ? (
          <div className="text-center py-24">
            ...
            <p className="text-sm text-gray-500 mb-6">
              We couldn't find this article in the client store.
            </p>
            <div className="flex items-center justify-center gap-3">
              <button
                onClick={() => navigate(-1)}
                className="px-4 py-2 rounded bg-emerald-50 text-emerald-700 border border-emerald-100"
              >
                Go back
              </button>
              <button
          ...
          >
            ...
            <h1 className="text-4xl md:text-6xl font-serif font-bold leading-[1.05] text-gray-900">
              {ui.title}
            </h1>
            ...
            <div className="flex items-center justify-between pt-8 border-t border-gray-100">
              ...
              <div className="flex gap-2">
                <button className="p-2 hover:bg-gray-50 rounded-full text-gray-400 hover:text-emerald-600 transition-colors">
                  <Bookmark size={20} />
                </button>
                <button className="p-2 hover:bg-gray-50 rounded-full text-gray-400 hover:text-emerald-600 transition-colors">
                  <Share2 size={20} />
      ...
      {blog && (
        <article className="max-w-3xl mx-auto px-6 pb-20">
          <motion.div
            ...
            className="prose prose-lg prose-slate prose-headings:font-serif prose-headings:font-bold max-w-none"
          >
      ...
      {blog && (
        <section className="bg-slate-50 border-t border-slate-200 py-20">
          <div className="max-w-5xl mx-auto px-6">
            <div className="flex items-end justify-between mb-10">
              <div>
                ...
                <p className="text-gray-500 mt-2">
                  Curated insights for your journey.
                </p>
              </div>
              <Link
                to="/blogs"
                className="hidden md:flex items-center gap-2 text-sm font-bold text-emerald-700 hover:text-emerald-800 group"
              >
                View all articles{" "}
                <ArrowRight
                  size={16}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {related.map((r, idx) => (
                <motion.div
                  ...
                  className={`bg-white rounded-2xl p-8 shadow-sm border hover:border-emerald-100 transition ${
                    idx === 0 ? "border-emerald-100" : ""
                  }`}
                >
                  <div>
                    ...
                    <h3 className="text-xl font-bold text-gray-900 mb-3 leading-tight font-serif">
                      {r.title}
                    </h3>
                    ...
                    <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                      <div className="flex items-center gap-2">
                        <div className="h-8 w-8 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 font-semibold">
                          {String(
                            r.author?.fullName || r.fullName || "A"
                          ).slice(0, 1)}
                        </div>
                        <div className="text-xs text-slate-500">
                          {r.author?.fullName || r.fullName || "Author"}
                        </div>
                      </div>
            ...
            <button
              className="md:hidden w-full mt-8 py-4 rounded-xl border border-slate-200 text-slate-600 font-bold hover:bg-white transition-colors"
              onClick={() => navigate("/blogs")}
            >
              View all articles
            </button>
