// src/components/BlogCreator.jsx
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white border-r border-gray-100 relative">
      ...
      <div className="px-8 py-6 border-b border-gray-50 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-serif font-bold text-[#1C1C1C]">Write Story</h1>
          <p className="text-xs text-gray-400 uppercase tracking-wider mt-1">Create new content</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-8">
        <div className="max-w-3xl mx-auto space-y-8">
          ...
          <div className="group">
            ...
            />
            {errors.tagline && <div className="text-xs text-red-600 mt-1">{errors.tagline}</div>}
          </div>
          ...
          <div className="mt-8 border border-gray-200 rounded-xl overflow-hidden shadow-sm">
            <div className="bg-gray-50/50 border-b border-gray-100 p-2 flex gap-1 flex-wrap">
              {tools.map((tool, idx) => (
                <button key={idx} type="button" className="p-2 rounded hover:bg-white hover:shadow-sm hover:text-emerald-600 text-gray-400 transition-all">
                  <tool.icon size={16} />
                </button>
              ))}
            </div>
            ...
            <textarea
              ...
              required
            />
            {errors.content && <div className="text-xs text-red-600 mt-2 px-6 pb-4">{errors.content}</div>}
          </div>
        </div>
      </div>
    </div>
...
/**
 * PreviewSection — right preview UI
 */
    ...
    <div className="hidden xl:flex w-[40%] h-full bg-[#F3F4F6] border-l border-gray-200 flex-col relative">
      <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-[#F3F4F6]">
        ...
        <div className="flex bg-gray-200/50 p-1 rounded-lg">
          <button onClick={() => setDevice("desktop")} className={`p-2 rounded-md transition-all ${device === "desktop" ? "bg-white text-gray-900 shadow-sm" : "text-gray-400 hover:text-gray-600"}`}>
            <Monitor size={16} />
          </button>
          <button onClick={() => setDevice("mobile")} className={`p-2 rounded-md transition-all ${device === "mobile" ? "bg-white text-gray-900 shadow-sm" : "text-gray-400 hover:text-gray-600"}`}>
            <Smartphone size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 flex justify-center items-start">
        <motion.div layout animate={{ width: device === "mobile" ? "375px" : "100%", scale: device === "mobile" ? 0.95 : 1 }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className={`bg-white shadow-xl border border-gray-100 overflow-hidden ${device === "mobile" ? "rounded-[40px] min-h-[812px]" : "rounded-xl min-h-[800px] w-full"}`}>
          {device === "desktop" && (
            <div className="h-8 bg-gray-50 border-b border-gray-100 flex items-center px-4 gap-2">
              ...
              <div className="mx-auto px-3 py-0.5 bg-white rounded text-[10px] text-gray-400 font-mono border border-gray-100">storyflow.com/blog/{data.slug || "..."}</div>
            </div>
          )}

          <div className={`bg-white ${device === "mobile" ? "px-6 py-8" : "px-12 py-12"}`}>
            <div className="flex items-center gap-3 mb-6">
              <span className="px-2 py-1 bg-emerald-50 text-emerald-700 text-[10px] font-bold uppercase tracking-wider rounded">{data.tagline || "Category"}</span>
              <span className="text-[10px] text-gray-400 font-medium uppercase">Oct 24, 2023</span>
            </div>

            <h1 className={`${device === "mobile" ? "text-3xl" : "text-5xl"} font-serif font-bold text-[#1C1C1C] leading-[1.1] mb-6`}>{data.title || "Your Title Here"}</h1>
            ...
            <div className="flex items-center gap-3 mb-8 pb-8 border-b border-gray-100">
              ...
              <div>
                <p className="text-xs font-bold text-gray-900">Gaurav Kaushik</p>
                <p className="text-[10px] text-gray-500">Senior Editor</p>
              </div>
            </div>

            <div className={`prose prose-slate prose-headings:font-serif prose-p:text-gray-600 prose-a:text-emerald-600 max-w-none font-serif ${device === "mobile" ? "prose-sm" : "prose-lg"}`}>
              {data.content ? data.content.split("\n").map((p, i) => <p key={i} className={p === "" ? "h-4" : ""}>{p}</p>) : (
...
/**
 * BlogCreator (main export)
 * - initial sample data
 * - publishes using useApi.createBlog
 * - shows field-level errors
 * - redirects to /dashboard and shows toast on success
 */
const BlogCreator = () => {
  const navigate = useNavigate();
  const { createBlog } = useApi();
  const addBlogToStore = useBlogStore((s) => s.addBlog); // not strictly necessary: createBlog already adds
...
By reading this article, you will learn what React is and how it can help you create powerful user interfaces.`,
  ...
  const validate = () => {
    const next = {};
    if (!formData.title || !formData.title.trim()) next.title = "Title is required";
    if (!formData.content || !formData.content.trim()) next.content = "Content is required";
  ...
  const handlePublish = async () => {
    ...
    try {
      // call createBlog (your hook sends cookies & updates store)
      const payload = {
      ...
      const created = await createBlog(payload); // createBlog will add to store via addBlog
      // If createBlog didn't add to store, optionally add:
      if (created && created._id && addBlogToStore) {
        try { addBlogToStore(created); } catch { /* ignore if createBlog already added */ }
      }
      ...
      toast.error(msg);
      // If backend returned field errors, try to map them (optional)
      // e.g. err.response?.data?.errors -> setErrors(...)
    } finally {
  ...
  return (
    <div className="flex h-screen w-full bg-[#FAFAFA] font-sans overflow-hidden selection:bg-emerald-100 selection:text-emerald-900">
      ...
      <main className="flex-1 flex relative shadow-2xl overflow-hidden">
        <EditorSection data={formData} onChange={handleInputChange} onPublish={handlePublish} errors={errors} isPublishing={isPublishing} />
