// src/components/SimpleEditModal.jsx
// NOTE: token screenshot reference (dev): /mnt/data/token.png
...
const SimpleEditModal = ({ isOpen = false, blog = null, onClose }) => {
  ...
  // Form State (initialized from blog prop)
  ...
  // store original snapshot for diff/rollback
  const originalRef = useRef({ title: "", content: "" });

  // sync form when blog prop changes
  useEffect(() => {
    setRemoteError(null);
    ...
    // keep original snapshot for diff/rollback
  ...
  // compute dirty state: title or content changed
  const isDirty = useMemo(() => {
    if (!blog) return false;
    const orig = originalRef.current;
    return (orig.title !== (formData.title || "")) || (orig.content !== (formData.content || ""));
  }, [formData, blog]);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!blog) return;
    // if not dirty, do nothing (should be disabled anyway)
    if (!isDirty) return;
    ...
    // keep a copy of original to rollback if needed
    const original = { ...(blog || {}) };
    ...
    try {
      ...
      // perform server update using hook
      const updated = await updateBlogById(id, payload);

      // updateBlogById should already update store; still, ensure canonical state
      if (updated) {
        if (updateBlogInStore) {
  ...
  // Framer Motion Variants (kept from your original)
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          {/* Toast container (safe to include here for local scope) */}
          {/* Toast container — keep here temporarily but ensure z-index is very high */}
<ToastContainer
  position="top-right"
  autoClose={3000}
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-zinc-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-zinc-900 leading-none">Edit Blog Post</h3>
                  <p className="text-xs text-zinc-500 mt-1">Update your content details below.</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold uppercase tracking-wider text-zinc-500 ml-1">Post Title</label>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold uppercase tracking-wider text-zinc-500 ml-1 flex items-center gap-1">
                  URL Slug <span className="bg-zinc-100 text-zinc-400 px-1.5 py-0.5 rounded text-[10px] font-normal border border-zinc-200">Immutable</span>
                </label>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold uppercase tracking-wider text-zinc-500 ml-1">Content</label>
                <div className="relative group">
              ...
              <div className="pt-2 flex items-center justify-between">
                <div className="text-xs text-zinc-400">Last edited: Just now</div>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-6 py-2 text-sm font-medium rounded-lg shadow-lg transition-all active:scale-95 disabled:opacity-70 ${
                      isLoading ? "bg-zinc-900 text-white" : isDirty ? "bg-zinc-900 hover:bg-black text-white" : "bg-zinc-200 text-zinc-500 cursor-not-allowed"
                    }`}
                  >
